...
const SYSTEM_PREAMBLE = `너는 사내 통합 업무 어시스턴트다(재고·장비·근태 관리). 아래 "할 수 있는 작업"과 "조회 가능 항목"을 보고, 사용자 요청에 맞는 작업·조회를 찾아 처리한다.
...
이때 id_ref 필드는 반드시 이름도 함께 넣어라(품목이면 itemName, 거래처면 partnerName, 위치면 locationName 등). 숫자 id는 틀려도 되지만 이름은 사용자가 말한 그대로 정확히 넣어라. 시스템이 이름으로 정확한 id를 확정한다. 이름 필드에는 사용자가 말한 이름을 글자 그대로 넣어라 — "evaporator_id"처럼 이름 뒤에 "_id"를 붙이거나 ID 형태로 바꾸지 마라. 장비명은 들은 그대로(예: "Evaporator") 넣어야 시스템이 찾을 수 있다.
...
function buildSystemPrompt(schemas: SchemaOp[]): string {
  ...
  lines.push('★ 기간 지정: my_attendance·my_stats·my_requests·external_work 는 기간을 함께 받을 수 있다. "이번주"=this_week, "지난주"=last_week, "이번달"=this_month, "지난달"=last_month, "올해"=this_year 를 params.period 에 넣는다. 특정 월은 params.yearMonth 에 "YYYY-MM"(예 "2026-05"). 미지정 시 이번달. 긴 기간(올해 등)은 목록이 길어지니 통계는 my_stats 를 권한다. 예: <<READ>>{"queryId":"my_attendance","params":{"period":"last_month"}}<<END>>');
